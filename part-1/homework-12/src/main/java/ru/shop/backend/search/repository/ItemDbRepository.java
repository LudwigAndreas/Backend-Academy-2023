...
import ru.shop.backend.search.model.db.ItemEntity;
...
public interface ItemDbRepository extends JpaRepository<ItemEntity, Long> {
    ...
    List<Object[]> findAllItemsByRegionIdAndIdIn(Integer regionId, List<Long> ids);

    @Query(value = "" +
            "select item_id from item_sku where sku = ?", nativeQuery = true)
    List<Integer> findAllItemsBySku(String parseInt);

    @Query(value = "" +
            "select i.* from item as i", nativeQuery = true)
    Stream<ItemEntity> findAllItems();
    ...
    List<Object[]> findAllCategoriesByIdIn(List<Integer> ids);
}
